...
import { GeneratePresignedUrl } from "../plugins/uploadFiles";
...
export function getPostGraphileOptions({
  ...
  const options: PostGraphileOptions<Request, Response> = {
    ...
    appendPlugins: [
      ...
      // a plugin to get s3 presigned URLa
      GeneratePresignedUrl,

      // Simplifies the field names generated by PostGraphile.
      PgSimplifyInflectorPlugin,

      // the filter connection plugin
    ...
    async pgSettings(req) {
      ...
      const tokenPayload = access_token
        ? (verify(access_token, process.env.JWT_SECRET!) as JwtPayload)
        : null;
