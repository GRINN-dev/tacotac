...
create function publ.tg_user_emails__forbid_if_verified() returns trigger as $$
begin
...
create trigger _200_forbid_existing_email before insert on publ.user_emails for each row execute procedure publ.tg_user_emails__forbid_if_verified();
...
/* 
drop function if exists priv.tg__send_verification_email cascade;
create function priv.tg__send_verification_email() returns trigger as $$
begin
  perform graphile_worker.add_job('user_emails__send_verification', json_build_object('email', NEW.email));
  return NEW;
end;
...
create trigger _900_send_verification_email
  ...
  execute procedure -- priv.tg__send_verification_email();
   priv.tg__add_job('user_emails__send_verification');
...
create function publ.tg_user_emails__prevent_delete_last_email() returns trigger as $$
begin
  if exists (
    with remaining as (
...
create function priv.tg_user_email_secrets__insert_with_user_email() returns trigger as $$
declare
